...
var InitDependableFields = function (options) {
  ...
  if (bootstrap3_enabled) {
    ...
    console.log("this plugin still requires bootstrap");
  }
  ...
  $(document).ready(function () {
    ...
    var dependenciesAsJquery = function (dependenciesId) {
      ...
      for (var i = 0; i < dependenciesId.length; i++) {
        ...
        //If input is radio btn group filter unchecked
        if (item.filter("input[type='radio']").size() > 0 || item.filter(':radio').size() > 0) {
          console.log("BEFORE");
          console.log(item);
          item = item.not(":not(':checked')");
          console.log("AFTER");
          console.log(item);
        }
      }
